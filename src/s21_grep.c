...
void cut_from_argv(char option, int i, int j, int *count,
                   char array[][FilenameSize], int argc, char **argv);
...
void file_processing(FILE *file, int files_count, char *filename, int *options,
                     ...
                     int template_files_count,
                     char template_files[][FilenameSize]);
char *pattern_searching(char *buffer, int templates_count,
                        char templates[][FilenameSize],
                        int template_files_count,
                        char template_files[][FilenameSize], int *options);
char *regular_searching(char *text, char *expr, int *options);
void line_by_line_output(char *buffer, char *filename, int line_number,
                         int files_count, int *options);
...
int main(int argc, char **argv) {
  ...
  if (files_count == 0) {  // console mode
    file_processing(stdin, 0, 0, options, templates_count, templates,
    ...
    for (int i = 0; i < files_count; i++) {
      ...
      if (file) {
        file_processing(file, files_count, files[i], options, templates_count,
                        templates, template_files_count, template_files);
      } else {
        if (!options[6])  // suppress error messages about nonexistent or
                          // unreadable files
...
void file_processing(FILE *file, int files_count, char *filename, int *options,
  ...
  while (fgets(buffer, BufferSize, file)) {
    line_number++;
    match_part =
        pattern_searching(buffer, templates_count, templates,
    ...
    if ((match && !options[0]) ||
        (!match && options[0])) {  // -v - invert match
      match_count++;
      if (!options[3] && !options[2])
        line_by_line_output(buffer, filename, line_number, files_count,
                            options);
    }
  }
  ...
  if (options[2]) {  // -c - output count of matching lines only
    ...
    if (files_count == 1 ||
        options[5]) {  // -h - suppress the prefixing of file names on output
      fprintf(stdout, "%d\n", match_count);
    } else {
...
void get_templates(int *templates_count, char templates[][FilenameSize],
  ...
  for (int i = 1; i < argc; i++) {  // templates extracting with -e and -f flags
    if (argv[i][0] == '-') {
      ...
      if (argv[i][j] == 'e') {
        ...
        cut_from_argv('f', i, j, template_files_count, template_files, argc,
                      argv);
      }
    }
  }
  if (*templates_count == 0 &&
      *template_files_count == 0) {  // if there's no -e or -f flags
  ...
  FILE *file;
  for (int i = 0; i < *template_files_count;
       i++) {  // checks if all of the template files exist
    file = fopen(template_files[i], "r");
    if (file == NULL) {
      fprintf(stderr, "s21_grep: %s: No such file or directory\n",
              template_files[i]);
...
void get_options(int *options, int argc, char **argv) {
  for (int i = 0; i < OptionsCount; i++) options[i] = 0;
  char temp;
